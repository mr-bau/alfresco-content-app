...
import { EMRBauDocumentCategory, IMRBauDocumentType, MrbauArchiveModel } from '../declaration/mrbau-doc-declarations';
import { EMRBauTaskCategory, MRBauTaskCategoryNames } from '../declaration/mrbau-task-declarations';
...
export class MrbauArchiveModelService {
  ...
  private _mrbauArchiveModel : MrbauArchiveModel | undefined ;
  ...
  getArchiveModelNodeTye(category:EMRBauDocumentCategory) : string | undefined
  {
  ...
  getArchiveModelNodeTitle(category:EMRBauDocumentCategory) : string | undefined
  {
    let docModel : IMRBauDocumentType[] = this.mrbauArchiveModel.mrbauArchiveModelTypes.filter( d => d.category == category);
    if (docModel.length > 0)
    {
