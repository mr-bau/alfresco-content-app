...
export function notAValidValueValidationMessage(err:any, field:any) {
  err;
  return `'${field.formControl.value}' ist keine gültige Angabe für dieses Feld.`;
}

export function autocompleteNotValidValidationMessage(err:any, field:any) {
  err;
...
export function netGrossTaxMismatchMessage(err:any, field:any) {
  field;
...
export function autocompleteValueFromListValidator(control: AbstractControl, field: FormlyFieldConfig, options : {[id: string]: any } | undefined): ValidationErrors | null {
  field;
  options;
...
export function germanParseFloat(value : string) : number | undefined {
  if (value == null) {
    return undefined;
  }
...
export function germanDecimalValidatorAndConverter(control: AbstractControl, field: FormlyFieldConfig, options : {[id: string]: any } | undefined): ValidationErrors | null {
  control;
  field;
  options;
  if (!control.value)
...
export function netGrossTaxRateValidatorAndConverter(control: AbstractControl, field: FormlyFieldConfig,  options : {[id: string]: any } | undefined): ValidationErrors | null {
  control;
  field;
  options;
...
export function regexValidator(control: AbstractControl, field: FormlyFieldConfig,  options : {[id: string]: any } | undefined): ValidationErrors | null {
  field;
  if (!control.value)
    return null;
  if (!options)
  ...
  return options.test(control.value) ? null : { 'pattern' : true };
}
...
export function dateFutureValidator(control: AbstractControl, field: FormlyFieldConfig, options : {[id: string]: any } | undefined): ValidationErrors | null {
  control;
  field;
  let msg:string;
  let days = 0;
  ...
  {
    msg = (days > 0) ? `Der Tag muss ${days} Tage in der Zukunft liegen.` : 'Der Tag muss in der Zukunft liegen.';
    return { 'date-future': { message: msg } }
  }
  return null;
}
