...
import { IMRBauTasksCategory, MRBauTask, EMRBauTaskStatus, EMRBauTaskCategory, IFileSelectData, ITaskChangedData} from '../../declaration/mrbau-task-declarations';
import { MrbauCommonService } from '../../services/mrbau-common.service';
import { TasksTableComponent } from '../task-table/taskstable.component';
...
import { SplitpaneComponent } from '../../splitpane/splitpane/splitpane.component';
...
@Component({
  ...
  selector: 'mrbau-tasks',
  templateUrl: './task-main.component.html',
  styleUrls: ['./task-main.component.scss'],

})
export class TaskMainComponent implements OnInit, CanComponentDeactivate {
  ...
  async testUser() {
    ...
    this.searchService.searchByQueryBody(searchRequest).subscribe(
      (nodePaging) => {
        let errorCount = 0;
        if (people && nodePaging.list?.entries) {
  ...
  initTaskCategories()
  {
    ...
    this.taskCategories = [
    {
      ...
      order: 'ORDER BY B.mrbt:dueDateValue ASC',
      searchRequest: {
        query: {
    ...
    },
    {
      ...
      tabName: 'Aufgaben',
      tabBadge: 0,
      order: 'ORDER BY B.cmis:creationDate ASC',
      searchRequest: {
  ...
  taskSelected(task : MRBauTask | null) {
  ...
  taskChanged(taskChangedData : ITaskChangedData)
  {
    ...
    if (this.tasksTableComponent != null) {
