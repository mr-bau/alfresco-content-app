...
import { MrbauUserinfoProfilePictureComponent } from "./mrbau-userinfo-profile-picture/mrbau-userinfo-profile-picture.component";
...
@Component({
  ...
  selector: 'mrbau-task-comment-block',
  template: `
    ...
    <ng-template #commentBlock>
      <div class="commentWrapper" *ngFor="let v of commentData; index as i; first as isFirst">
        <div class="commentInlineContainer" [ngClass]="(v.createdById == this.currentUserId) ? 'own' : 'other'">
          <mrbau-userinfo-profile-picture class="inlineIcon" [userName]="v.createdBy||''" [avatarId]="v.avatarId||''"></mrbau-userinfo-profile-picture>
