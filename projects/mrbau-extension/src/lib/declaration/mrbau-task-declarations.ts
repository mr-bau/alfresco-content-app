...
import { ResultNode, UserInfo } from '@alfresco/js-api';
...
@Pipe({name: 'mrbauTaskCategory', standalone: true})
export class MRBauTaskCategoryPipe implements PipeTransform {
  transform(value: EMRBauTaskCategory): string {
    ...
    return MRBauTaskCategoryNames[value as keyof typeof MRBauTaskCategoryNames] ? MRBauTaskCategoryNames[value as keyof typeof MRBauTaskCategoryNames] : value.toString();
  }
}
...
export class MRBauTask {
  ...
  public updateWithNodeData(node: ResultNode){
    this.id = node.id;
