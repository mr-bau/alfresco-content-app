...
import { MRBauTask, EMRBauTaskCategory} from '../../declaration/mrbau-task-declarations';
...
@Component({
  ...
  selector: 'mrbau-new-task-dialog',
  template: `
...
export class MrbauNewTaskDialogComponent extends MrbauBaseDialogComponent implements OnInit {
  ...
  private _oldCategory : EMRBauTaskCategory | null = null;
  ...
  override ngOnInit(): void {
    this.mrbauCommonService.patchFormFieldConfigArrayPristineUntouchedRecursive(this.fields);
  ...
  override modelChangeEvent()
  {
    ...
    if (cat && cat != this._oldCategory)
    {
      ...
      if (cat > EMRBauTaskCategory.CommonTaskStart && cat < EMRBauTaskCategory.CommonTaskLast)
      {
        ...
        (this.form.get('mrbt:dueDateValue') as any).patchValue(this.mrbauCommonService.getFormDateValue(date));
        ...
        if (cat == EMRBauTaskCategory.CommonTaskInfo)
        {
          ...
          (this.form.get('mrbt:dueDateValue') as any).patchValue(undefined);
        }
