...
@Component({
  ...
  selector: 'mrbau-tag-done-docs-dialog',
  template: `
  <h2 mat-dialog-title>Verknüpfte Dokumente mit "Erledigt" markieren</h2>
  <p class="mat-dialog-text">Alle mit dem Haupt-Dokument verknüpften Dokumente erhalten den Tag "Erledigt".</p>
  <mat-dialog-content>
    <form [formGroup]="form">
  ...
  <mat-dialog-actions align="end">
    ...
    <button mat-button color="primary" [mat-dialog-close]="true" [disabled]="formIsInValid()">Tag Anwenden</button>
...
export class MrbauTagDoneLinkedDocsDialogComponent extends MrbauBaseDialogComponent implements OnInit {
  nodes : Node[] = [];
      ...
      fieldGroup: [
        {
          ...
          props: {
            ...
            placeholder: 'Tag für alle Dokument erstellen',
  ...
  async onDialogClose(result : boolean)
  {
    if (result)
    {
      if (!this.mrbauCommonService.isTagManagerUser())
      {
        this.mrbauCommonService.showError("Keine ausreichende Berechtigung für Benutzer!");
        return;
      }
      ...
      try {
        for (let i=0; i< this.nodes.length; i++)
        {
          ...
          if (targestAssociations?.list?.entries) {
          ...
          for (let i=0; i<docs.length; i++) {
            await this.mrbauCommonService.addTag(docs[i].id, this.mrbauCommonService.DEFAULT_TAG_ERLEDIGT.toLowerCase());
          }
        }
        this.mrbauCommonService.showInfo("Tags wurden erfolgreich angewendet");
      } catch(error : any) {
        console.log(error);
